    ...
    while (recv(new_socket, recv_buf, sizeof(recv_buf), 0) > 0 ){
        ...
        try {
            ...
            if (v.size() >= 4){
                ...
                char buffer[100];
                char pthread_self_value_char = sprintf(buffer, "%ld", pthread_self());
                string pthread_self_value_string(100, pthread_self_value_char);


                string msg;
                if (transaction_result == 0){
                    msg = "From server: tid(" + pthread_self_value_string + ") Transaction " + v[0] + " " + v[1] + " " + transaction_type + " " + v[3] + " has been finished.";
                } else if (transaction_result == -1){
                    msg = "From server: tid(" + pthread_self_value_string + ") Balance not enough. Transaction " + v[0] + " " + v[1] + " " + transaction_type + " " + v[3] + " failed.";
                } else {
                    msg = "From server: tid(" + pthread_self_value_string + ") Transaction " + v[0] + " " + v[1] + " " + transaction_type + " " + v[3] + " has failed.";
                }
                send(new_socket , msg.c_str(), msg.length()+1, 0 );
                cout << "Server: sent to client tid(" + pthread_self_value_string + ") with msg '" << msg << "'\n" << endl;
